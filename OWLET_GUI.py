...
class OWLET_Interface(object):
    ...
    def display_GUI(self):    
            ...
            for video in self.videos:
                ...
                if self.taskVideo is not None and len(self.taskVideo) == 1:
                    ...
                    if not self.override_audio_matching:
                        found_match = owlet.match_audio(videotest, self.taskVideo)
        ...
        lblblank.grid(row = 4, columnspan = 6)
        
      
        # audio_var = tk.IntVar()
        # audio_var.set(self.audio_var)
        # audiostate = self.audiostate
        # audio_checkbox = tk.Checkbutton(window, text = "Match audio to find task start",  variable = audio_var, fg="white",
        #                     onvalue=1, offvalue=0, bg = "#3E4149", state=audiostate, command=match_audio_checkbox)
        # audio_checkbox.grid(column = 1, columnspan=4, row = 15)
